...
public class SpeedUpgradeInventory extends InventoryUpgradeGUI {
    private final UltraChest ultraChest;
    private final UltraStorage plugin = UltraStorage.getInstance();

    public SpeedUpgradeInventory(UltraChest ultraChest) {
        super();
    ...
    protected Inventory createInventory() {
        return Bukkit.createInventory(null, 3 * 9, TextUtils.color("Speed upgrade"));
    }
    ...
    public void decorate(Player player) {
        ...
        InventoryButton button =
                new InventoryButton().creator(player1 -> getButton(Objects.requireNonNull(GuiConfig.getStringName(ConfigPath.GUI_UPGRADES_SPEED_ITEM)), PlaceholderUtils.replacePlaceholders(GuiConfig.getStringListLore(ConfigPath.GUI_UPGRADES_SPEED_ITEM), ultraChest))).consumer(event -> {
