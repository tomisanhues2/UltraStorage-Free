...
public class SlotUpgradeInventory extends InventoryUpgradeGUI {
    ...
    protected Inventory createInventory() {
        return Bukkit.createInventory(null, 3 * 9, TextUtils.color("Slot upgrade"));
    }
    ...
    public void decorate(Player player) {
        ...
        InventoryButton button =
                new InventoryButton().creator(player1 -> getButton(GuiConfig.getStringMaterial(ConfigPath.GUI_UPGRADES_SLOT_ITEM), TextUtils.color(" "), PlaceholderUtils.replacePlaceholders(GuiConfig.getStringListLore(ConfigPath.GUI_UPGRADES_SLOT_ITEM), ultraChest))).consumer(event -> {
                });
