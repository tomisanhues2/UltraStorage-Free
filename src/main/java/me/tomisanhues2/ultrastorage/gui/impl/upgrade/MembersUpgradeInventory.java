...
public class MembersUpgradeInventory extends InventoryUpgradeGUI {
    private final UltraChest ultraChest;
    private final UltraStorage plugin = UltraStorage.getInstance();

    public MembersUpgradeInventory(UltraChest ultraChest) {
        this.ultraChest = ultraChest;
    }

    @Override
    protected Inventory createInventory() {
        return Bukkit.createInventory(null, 3 * 9, TextUtils.color("Members upgrade"));
    }
    ...
    public void decorate(Player player) {
        ...
        InventoryButton button =
                new InventoryButton().creator(player1 -> getButton(GuiConfig.getStringMaterial(ConfigPath.GUI_UPGRADES_MEMBERS_ITEM), TextUtils.color(" "), PlaceholderUtils.replacePlaceholders(GuiConfig.getStringListLore(ConfigPath.GUI_UPGRADES_MEMBERS_ITEM), ultraChest))).consumer(event -> {
                });
