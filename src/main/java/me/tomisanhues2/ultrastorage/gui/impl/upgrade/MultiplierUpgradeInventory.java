...
public class MultiplierUpgradeInventory extends InventoryUpgradeGUI {
    private final UltraChest ultraChest;
    private final UltraStorage plugin = UltraStorage.getInstance();

    public MultiplierUpgradeInventory(UltraChest ultraChest) {
        super();
    ...
    protected Inventory createInventory() {
        return Bukkit.createInventory(null, 3 * 9, TextUtils.color("Multiplier upgrade"));
    }
    ...
    public void decorate(Player player) {
        ...
        InventoryButton button =
                new InventoryButton().creator(player1 -> getButton(GuiConfig.getStringMaterial(ConfigPath.GUI_UPGRADES_MULTIPLIER_ITEM), TextUtils.color(" "), PlaceholderUtils.replacePlaceholders(GuiConfig.getStringListLore(ConfigPath.GUI_UPGRADES_MULTIPLIER_ITEM), ultraChest))).consumer(event -> {
                });
